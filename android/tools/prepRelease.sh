...
#get_version_notes "android" "${BUILD_NUMBER}" "$TIER" > $CHANGELOG_PATH
...
KMEA_DST_NAME="keyman-engine-android-${BUILD_NUMBER}.zip"
KMEA_DST="${WORK_DIR}/${UPLOAD_DIR}/${KMEA_DST_NAME}"

KEYMAN_SAMPLES="samples"

echo "engine dest: $KMEA_DST"

cd "$WORK_DIR"

echo "Copying Keyman Engine samples into ${UPLOAD_DIR}/${KMEA_DST_NAME}..."
cp -rf "${KEYMAN_SAMPLES}" "${UPLOAD_DIR}/samples"
cd "${UPLOAD_DIR}"
zip -qr "${KMEA_DST_NAME}" "samples"
rm -rf "samples"
cd "$WORK_DIR"
...
KEYMANAPP_APK="build/Build/Products/Release-iphoneos/Keyman.ipa"
KEYMANAPP_APK_DST="keyman-android-${BUILD_NUMBER}.ipa"

echo "Copying Keyman IPA ${KEYMANAPP_APK} => ${UPLOAD_DIR}/${KEYMANAPP_APK_DST}..."
cp "${KEYMANAPP_APK}" "${WORK_DIR}/${UPLOAD_DIR}/${KEYMANAPP_APK_DST}"

KEYMANAPP_SIM_FOLDER="build/Build/Products/Release-iphonesimulator"
KEYMANAPP_SIM_APP="$KEYMANAPP_SIM_FOLDER/Keyman.app"
KEYMANAPP_SIM_APP_DST="keyman-android-simulator-${BUILD_NUMBER}.app.zip"
...
if [ "${RELEASE_OEM_FIRSTVOICES}" = true ]; then
  FIRSTVOICESAPP_IPA="../oem/firstvoices/android/build/Build/Products/Release-iphoneos/FirstVoices.ipa"
  FIRSTVOICESAPP_IPA_DST="firstvoices-android-${BUILD_NUMBER}.ipa"
  ...
  FIRSTVOICESAPP_SIM_FOLDER="../oem/firstvoices/android/build/Build/Products/Release-iphonesimulator/"
  FIRSTVOICESAPP_SIM_APP="$FIRSTVOICESAPP_SIM_FOLDER/FirstVoices.app"
  FIRSTVOICESAPP_SIM_APP_DST="firstvoices-android-simulator-${BUILD_NUMBER}.app.zip"
...
write_download_info "Keyman Engine for android" "${KMEA_DST_NAME}" "${BUILD_NUMBER}" "${TIER}" "android"
write_download_info "Keyman for iPhone and iPad" "${KEYMANAPP_APK_DST}" "${BUILD_NUMBER}" "${TIER}" "android"

if [ ${RELEASE_OEM_FIRSTVOICES} = true ]; then
  write_download_info "FirstVoices Keyboards" "${FIRSTVOICESAPP_IPA_DST}" "${BUILD_NUMBER}" "${TIER}" "android"
fi
